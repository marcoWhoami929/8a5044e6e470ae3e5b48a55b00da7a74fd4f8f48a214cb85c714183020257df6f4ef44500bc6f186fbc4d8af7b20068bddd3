            ...
            <table class="table table-responsive table-striped table-hover " id="tableVentasProducto">
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style='font-weight:bold;text-align:right' onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[0]; ?>','<?= $_REQUEST['anio'] ?>','0','<?= $row['CCODIGOPRODUCTO']; ?>')">$ <?= number_format($row[(int)$arreglo[0]], 2) ?></td>
                            <td style='font-weight:bold;text-align:right' onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[1]; ?>','<?= $_REQUEST['anio'] ?>','0','<?= $row['CCODIGOPRODUCTO']; ?>')">$ <?= number_format($row[(int)$arreglo[1]], 2) ?></td>
                            <td style='font-weight:bold;text-align:right' onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[2]; ?>','<?= $_REQUEST['anio'] ?>','0','<?= $row['CCODIGOPRODUCTO']; ?>')">$ <?= number_format($row[(int)$arreglo[2]], 2) ?></td>
                            <td style='font-weight:bold;text-align:right' onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[3]; ?>','<?= $_REQUEST['anio'] ?>','0','<?= $row['CCODIGOPRODUCTO']; ?>')">$ <?= number_format($row[(int)$arreglo[3]], 2) ?></td>
                            <td style='font-weight:bold;text-align:right' onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[4]; ?>','<?= $_REQUEST['anio'] ?>','0','<?= $row['CCODIGOPRODUCTO']; ?>')">$ <?= number_format($row[(int)$arreglo[4]], 2) ?></td>
                            <td style='font-weight:bold;text-align:right' onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[5]; ?>','<?= $_REQUEST['anio'] ?>','0','<?= $row['CCODIGOPRODUCTO']; ?>')">$ <?= number_format($row[(int)$arreglo[5]], 2) ?></td>
                            <th style="font-weight:bold;text-align:right" onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[6]; ?>','<?= $_REQUEST['anio'] ?>','0','<?= $row['CCODIGOPRODUCTO']; ?>')">$<?= number_format($row['Totales'], 2) ?></th>
                        </tr>
                    <?php
                        $finales++;
                    }
                ...
                <tfoot>
                    <tr>
                        <th>Total General</th>
                        <th></th>
                        <th style="font-weight:bold;text-align:right" onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[0]; ?>','<?= $_REQUEST['anio'] ?>','0','')">$<?= number_format($numDia1, 2) ?></th>
                        <th style="font-weight:bold;text-align:right" onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[1]; ?>','<?= $_REQUEST['anio'] ?>','0','')">$<?= number_format($numDia2, 2) ?></th>
                        <th style="font-weight:bold;text-align:right" onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[2]; ?>','<?= $_REQUEST['anio'] ?>','0','')">$<?= number_format($numDia3, 2) ?></th>
                        <th style="font-weight:bold;text-align:right" onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[3]; ?>','<?= $_REQUEST['anio'] ?>','0','')">$<?= number_format($numDia4, 2) ?></th>
                        <th style="font-weight:bold;text-align:right" onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[4]; ?>','<?= $_REQUEST['anio'] ?>','0','')">$<?= number_format($numDia5, 2) ?></th>
                        <th style="font-weight:bold;text-align:right" onclick="redireccionAccionVentas('','Productos','<?= $arreglo2[5]; ?>','<?= $_REQUEST['anio'] ?>','0','')">$<?= number_format($numDia6, 2) ?></th>
            ...
            <table class="table table-responsive table-striped table-hover " id="tableVentasProducto">
                <thead>
                    <tr>
                        ...
                        for ($i = 1; $i < 7; $i++) {
                            echo "<th>" . date('Y-m-d', strtotime($año . "W" . str_pad($week, 2, '0', STR_PAD_LEFT) . $i)) . "</th>";
                        }
                        ?>
